...
class Config:
    def __init__(self):
        ...
        # Z-score標準化設定（NEW）
        # ================================
        self.use_zscore_normalization = True  # Z-score標準化を使用
        self.zscore_epsilon = 1e-8  # 標準偏差が小さい場合の安定化項
        ...
        # 血行動態信号タイプ設定（SBPに変更）
        self.signal_type = "SBP"  # SBP (収縮期血圧)
        self.signal_prefix = "SBP_s2"  # ファイル名のプレフィックス
        self.signal_range = (60, 210)  # SBPの典型的な範囲 (mmHg)
...
def stratified_sampling_split(task_rgb, task_signal, val_ratio=0.1, n_strata=5, method='quantile'):
    ...
    if len(strata_info) > 0:
        ...
        for info in strata_info:
            print(f"        層{info['stratum_id']}: "
                  f"SBP[{info['signal_range'][0]:.1f}, {info['signal_range'][1]:.1f}] mmHg "
    ...
    print(f"      SBP値の分布確認:")
    print(f"        元データ: 平均={signal_mean:.1f}mmHg, 標準偏差={signal_std:.1f}mmHg")
    print(f"        訓練データ: 平均={train_signals.mean():.1f}mmHg, 標準偏差={train_signals.std():.1f}mmHg")
    print(f"        検証データ: 平均={val_signals.mean():.1f}mmHg, 標準偏差={val_signals.std():.1f}mmHg")
...
class SBPDataset(Dataset):
    def __init__(self, rgb_data, signal_data, model_type='3d', 
        ...
        normalizer: ZScoreNormalizer インスタンス（標準化用）
        """
...
def create_model(config):
    ...
    if config.verbose:
        ...
        print(f"信号タイプ: {config.signal_type} (範囲: {config.signal_range[0]}-{config.signal_range[1]} mmHg)")
        print(f"Z-score標準化: {'有効' if config.use_zscore_normalization else '無効'}")
        if config.use_lab:
...
def load_data_single_subject(subject, config):
    ...
    # SBPデータの範囲確認
    print(f"  SBPデータ範囲: [{signal_data.min():.1f}, {signal_data.max():.1f}] mmHg")
    print(f"  SBP平均: {signal_data.mean():.1f} mmHg, 標準偏差: {signal_data.std():.1f} mmHg")
...
def load_all_subjects_data(config):
    """全被験者のデータを読み込み（Cross-Subject用、SBP対応）"""
    all_rgb_data = []
    all_signal_data = []
    subject_task_info = []
    ...
    print(f"  全体SBP統計: 平均={all_signal_data.mean():.1f}mmHg, 標準偏差={all_signal_data.std():.1f}mmHg")
    
    return all_rgb_data, all_signal_data, subject_task_info

# ================================
# 学習関数（AMP・Z-score標準化対応版）
# ================================
def train_model(model, train_loader, val_loader, config, fold=None, subject=None, normalizer=None):
    """モデルの学習（AMP + Warmup + Z-score標準化対応）"""
    ...
    if config.verbose:
        ...
        if normalizer is not None:
            print(f"    Z-score標準化パラメータ: μ={normalizer.mean:.1f}, σ={normalizer.std:.1f}")
    ...
    for epoch in range(config.epochs):
        ...
        # MAEは元のmmHg単位で計算（逆変換後）
        if normalizer is not None and config.use_zscore_normalization:
            val_preds_mmhg = normalizer.inverse_transform(val_preds_flat)
            val_targets_mmhg = normalizer.inverse_transform(val_targets_flat)
            val_mae = mean_absolute_error(val_targets_mmhg, val_preds_mmhg)
        else:
        ...
        if config.verbose and ((epoch + 1) % 20 == 0 or epoch == 0 or epoch < config.warmup_epochs):
            ...
            print(f"      Val Loss: {val_loss:.4f}, MAE: {val_mae:.2f}mmHg, Corr: {val_corr:.4f}")
...
# 評価関数（Z-score逆変換対応）
# ================================
def evaluate_model(model, test_loader, config, normalizer=None):
    """モデルの評価（AMP + Z-score逆変換対応）"""
    model.eval()
    predictions = []
    targets = []
    ...
    predictions = np.concatenate(predictions, axis=0)  # (N, T) - 標準化されたまま
    targets = np.concatenate(targets, axis=0)  # (N, T) - 標準化されたまま
    
    # Z-score逆変換（元のmmHg単位に戻す）
    if normalizer is not None and config.use_zscore_normalization:
        predictions_mmhg = normalizer.inverse_transform(predictions)
        targets_mmhg = normalizer.inverse_transform(targets)
    else:
        predictions_mmhg = predictions
        targets_mmhg = targets
    
    # 全体のメトリクス計算（mmHg単位）
    mae = mean_absolute_error(targets_mmhg.flatten(), predictions_mmhg.flatten())
    rmse = np.sqrt(np.mean((targets_mmhg.flatten() - predictions_mmhg.flatten()) ** 2))
    
    # 相関は標準化の影響を受けないので、どちらで計算してもOK
    corr, p_value = pearsonr(targets.flatten(), predictions.flatten())
    
    # R^2も元のスケールで計算
    ss_res = np.sum((targets_mmhg.flatten() - predictions_mmhg.flatten()) ** 2)
    ss_tot = np.sum((targets_mmhg.flatten() - np.mean(targets_mmhg.flatten())) ** 2)
    r2 = 1 - (ss_res / ss_tot)
    
    return {
        'mae': mae, 'rmse': rmse, 'corr': corr,
        'r2': r2, 'p_value': p_value,
        'predictions': predictions_mmhg,  # mmHg単位
        'targets': targets_mmhg  # mmHg単位
    }

# ================================
# Cross-Subject 5分割交差検証（Z-score標準化対応版）
# ================================
def cross_subject_cv(config):
    """被験者間5分割交差検証（Z-score標準化対応）"""
    ...
    with open(split_info_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"信号タイプ: {config.signal_type} (収縮期血圧)\n")
        f.write(f"Z-score標準化: {'有効' if config.use_zscore_normalization else '無効'}\n")
        ...
        f.write(f"SBP全体統計: 平均={all_signal_data.mean():.1f}mmHg, 標準偏差={all_signal_data.std():.1f}mmHg\n\n")
    ...
    for fold_idx in range(config.n_folds):
        ...
        # Z-score標準化（訓練データのみから平均・標準偏差を計算）
        normalizer = None
        if config.use_zscore_normalization:
            normalizer = ZScoreNormalizer()
            normalizer.fit(train_signal.flatten())  # 訓練データから統計量を計算
            
            # 全データを標準化
            train_signal = normalizer.transform(train_signal)
            val_signal = normalizer.transform(val_signal)
            test_signal = normalizer.transform(test_signal)
            
            print(f"  Z-score標準化適用:")
            print(f"    訓練データ統計: μ={normalizer.mean:.1f}mmHg, σ={normalizer.std:.1f}mmHg")
            print(f"    標準化後の範囲: [{train_signal.min():.3f}, {train_signal.max():.3f}]")
        ...
        # データローダー作成
        train_dataset = SBPDataset(train_rgb, train_signal, config.model_type, 
                                   config.use_channel, config, is_training=True)
        val_dataset = SBPDataset(val_rgb, val_signal, config.model_type, 
                                 config.use_channel, config, is_training=False)
        test_dataset = SBPDataset(test_rgb, test_signal, config.model_type, 
                                  config.use_channel, config, is_training=False)
        ...
        model, train_preds, train_targets = train_model(model, train_loader, val_loader, 
                                                       config, fold=fold_idx, normalizer=normalizer)
        
        # 評価（逆変換してmmHg単位でメトリクス計算）
        train_results = evaluate_model(model, train_loader, config, normalizer)
        test_results = evaluate_model(model, test_loader, config, normalizer)
        
        print(f"\n  Fold {fold_idx + 1} 結果:")
        print(f"    訓練 - MAE: {train_results['mae']:.2f}mmHg, Corr: {train_results['corr']:.4f}")
        print(f"    テスト - MAE: {test_results['mae']:.2f}mmHg, Corr: {test_results['corr']:.4f}")
        ...
        fold_results.append({
            ...
            'train_predictions': train_results['predictions'],  # mmHg単位
            'train_targets': train_results['targets'],  # mmHg単位
            'test_predictions': test_results['predictions'],  # mmHg単位
            'test_targets': test_results['targets'],  # mmHg単位
            'normalizer_mean': normalizer.mean if normalizer else None,
            'normalizer_std': normalizer.std if normalizer else None
        })
    ...
    print(f"\n{'='*60}")
    print("5分割交差検証 最終結果（SBP）")
    print(f"{'='*60}")
    print(f"訓練平均: MAE={avg_train_mae:.2f}mmHg, Corr={avg_train_corr:.4f}")
    print(f"テスト平均: MAE={avg_test_mae:.2f}±{std_test_mae:.2f}mmHg, Corr={avg_test_corr:.4f}±{std_test_corr:.4f}")
    
    # 結果をCSVに保存
    results_df = pd.DataFrame([{
        'Fold': r['fold'],
        'Train_MAE_mmHg': r['train_mae'],
        'Train_Corr': r['train_corr'],
        'Test_MAE_mmHg': r['test_mae'],
        'Test_Corr': r['test_corr'],
        'Normalizer_Mean': r['normalizer_mean'],
        'Normalizer_Std': r['normalizer_std']
    } for r in fold_results])
    
    # 平均と標準偏差を追加
    mean_row = pd.DataFrame({
        'Fold': ['Mean'],
        'Train_MAE_mmHg': [avg_train_mae],
        'Train_Corr': [avg_train_corr],
        'Test_MAE_mmHg': [avg_test_mae],
        'Test_Corr': [avg_test_corr],
        'Normalizer_Mean': [np.mean([r['normalizer_mean'] for r in fold_results if r['normalizer_mean']])],
        'Normalizer_Std': [np.mean([r['normalizer_std'] for r in fold_results if r['normalizer_std']])]
    })
    
    std_row = pd.DataFrame({
        'Fold': ['Std'],
        'Train_MAE_mmHg': [np.std([r['train_mae'] for r in fold_results])],
        'Train_Corr': [np.std([r['train_corr'] for r in fold_results])],
        'Test_MAE_mmHg': [std_test_mae],
        'Test_Corr': [std_test_corr],
        'Normalizer_Mean': [np.std([r['normalizer_mean'] for r in fold_results if r['normalizer_mean']])],
        'Normalizer_Std': [np.std([r['normalizer_std'] for r in fold_results if r['normalizer_std']])]
    })
    
    results_df = pd.concat([results_df, mean_row, std_row], ignore_index=True)
    results_df.to_csv(save_dir / 'cross_validation_results_SBP.csv', index=False)

# ================================
# Within-Subject 6分割交差検証（Z-score標準化対応）
# ================================
def task_cross_validation(rgb_data, signal_data, config, subject, subject_save_dir):
    """タスクごとの6分割交差検証（Within-Subject用、Z-score標準化対応）"""
    ...
    for fold, test_task in enumerate(config.tasks):
        ...
        if config.use_zscore_normalization:
            normalizer = ZScoreNormalizer()
            normalizer.fit(train_signal)  # 訓練データから統計量を計算
            
            # 全データを標準化
            train_signal_normalized = normalizer.transform(train_signal)
            val_signal_normalized = normalizer.transform(val_signal)
            test_signal_normalized = normalizer.transform(test_signal)
            
            print(f"    Z-score標準化:")
            print(f"      訓練データ: μ={normalizer.mean:.1f}mmHg, σ={normalizer.std:.1f}mmHg")
            print(f"      標準化後範囲: [{train_signal_normalized.min():.3f}, {train_signal_normalized.max():.3f}]")
        else:
            train_signal_normalized = train_signal
            val_signal_normalized = val_signal
            test_signal_normalized = test_signal
        ...
        train_dataset = SBPDataset(train_rgb[np.newaxis, ...], train_signal_normalized[np.newaxis, ...], 
                                   config.model_type, config.use_channel, config, is_training=True)
        val_dataset = SBPDataset(val_rgb[np.newaxis, ...], val_signal_normalized[np.newaxis, ...], 
                                 config.model_type, config.use_channel, config, is_training=False)
        test_dataset = SBPDataset(test_rgb[np.newaxis, ...], test_signal_normalized[np.newaxis, ...], 
                                  config.model_type, config.use_channel, config, is_training=False)
        ...
        model, train_preds, train_targets = train_model(
            model, train_loader, val_loader, config, fold, subject, normalizer
        )
        ...
        if train_preds is not None and train_targets is not None:
            if normalizer is not None and config.use_zscore_normalization:
                train_preds_mmhg = normalizer.inverse_transform(train_preds.flatten())
                train_targets_mmhg = normalizer.inverse_transform(train_targets.flatten())
            else:
                train_preds_mmhg = train_preds.flatten()
                train_targets_mmhg = train_targets.flatten()
            
            train_mae = mean_absolute_error(train_targets_mmhg, train_preds_mmhg)
            train_corr = np.corrcoef(train_targets.flatten(), train_preds.flatten())[0, 1]
        else:
            train_mae = None
            train_corr = None
            train_preds_mmhg = None
            train_targets_mmhg = None
        
        if config.verbose:
            if train_mae is not None and train_corr is not None:
                print(f"    Train: MAE={train_mae:.2f}mmHg, Corr={train_corr:.4f}")
            print(f"    Test:  MAE={test_results['mae']:.2f}mmHg, Corr={test_results['corr']:.4f}")
        ...
        fold_results.append({
            'fold': fold + 1,
            'test_task': test_task,
            'train_predictions': train_preds_mmhg if train_preds_mmhg is not None else train_preds,  # mmHg単位
            'train_targets': train_targets_mmhg if train_targets_mmhg is not None else train_targets,  # mmHg単位
            'test_predictions': test_results['predictions'],  # mmHg単位
            'test_targets': test_results['targets'],  # mmHg単位
            'train_mae': train_mae,
...
def plot_fold_results_cross_subject(result, fold_dir, config):
    """各Foldの結果をプロット（Cross-Subject用、SBP単位）"""
    fold = result['fold']
    ...
    plt.xlabel('真値 (mmHg)')
    plt.ylabel('予測値 (mmHg)')
    plt.title(f"Fold {fold} 訓練データ (SBP) - MAE: {result['train_mae']:.2f}mmHg, Corr: {result['train_corr']:.3f}")
    ...
    plt.xlabel('真値 (mmHg)')
    plt.ylabel('予測値 (mmHg)')
    plt.title(f"Fold {fold} テストデータ (SBP) - MAE: {result['test_mae']:.2f}mmHg, Corr: {result['test_corr']:.3f}")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_all_folds_summary(fold_results, save_dir, config):
    ...
    avg_train_mae = np.mean([r['train_mae'] for r in fold_results])
    avg_train_corr = np.mean([r['train_corr'] for r in fold_results])
    plt.title(f'全Fold 訓練データ (SBP) - 平均MAE: {avg_train_mae:.2f}mmHg, 平均Corr: {avg_train_corr:.3f}')
    ...
    avg_test_mae = np.mean([r['test_mae'] for r in fold_results])
    avg_test_corr = np.mean([r['test_corr'] for r in fold_results])
    plt.title(f'全Fold テストデータ (SBP) - 平均MAE: {avg_test_mae:.2f}mmHg, 平均Corr: {avg_test_corr:.3f}')
    ...
    ax1.axhline(y=np.mean(mae_values), color='r', linestyle='--', 
                label=f'平均: {np.mean(mae_values):.2f}mmHg')
    ax1.set_ylabel('MAE (mmHg)')
    ax1.set_title('各FoldのMAE (SBP)')
    ax1.legend()
...
def plot_fold_results_colored(result, save_dir, config):
    ...
    plt.xlabel('真値 (mmHg)')
    plt.ylabel('予測値 (mmHg)')
    plt.title(f"Fold {fold} テストデータ ({test_task}) (SBP) - MAE: {result['test_mae']:.2f}mmHg, Corr: {result['test_corr']:.3f}")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def plot_subject_summary_colored(fold_results, all_test_predictions, all_test_targets, 
    ...
    if all_train_predictions:
        ...
        plt.xlabel('真値 (mmHg)')
        plt.ylabel('予測値 (mmHg)')
        plt.title(f"{subject} 全訓練データ (SBP) - MAE: {all_train_mae:.2f}mmHg, Corr: {all_train_corr:.3f}")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
    ...
    plt.xlabel('真値 (mmHg)')
    plt.ylabel('予測値 (mmHg)')
    plt.title(f"{subject} 全テストデータ (SBP) - MAE: {all_test_mae:.2f}mmHg, Corr: {all_test_corr:.3f}")
...
def plot_all_subjects_summary_unified(all_subjects_results, config):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'all_subjects_performance_comparison_SBP.png', dpi=150, bbox_inches='tight')
    plt.close()

# ================================
# メイン実行（高速化版・Z-score標準化対応）
# ================================
def main():
    config = Config()
    ...
    print("\n" + "="*60)
    print(" PhysNet2DCNN - SBP予測（Z-score標準化対応）")
    print("="*60)
    print(f"モード: {config.model_mode}")
    print(f"血行動態信号: {config.signal_type} (範囲: {config.signal_range[0]}-{config.signal_range[1]} mmHg)")
    print(f"Z-score標準化: {'有効' if config.use_zscore_normalization else '無効'}")
    ...
    print(f"\n【Z-score標準化】")
    print(f"  状態: {'有効' if config.use_zscore_normalization else '無効'}")
    if config.use_zscore_normalization:
        print(f"  方法: 各foldの訓練データから平均・標準偏差を計算")
        print(f"  適用: 訓練・検証・テストデータ全てに適用")
        print(f"  評価: mmHg単位に逆変換してMAE/RMSE計算")
    ...
    if config.model_mode == 'cross_subject':
        ...
        for subj_idx, subject in enumerate(config.subjects):
            ...
            try:
                ...
                print(f"\n  {subject} 完了:")
                print(f"    全体訓練: MAE={train_mae:.2f}mmHg, Corr={train_corr:.4f}")
                print(f"    全体テスト: MAE={test_mae:.2f}mmHg, Corr={test_corr:.4f}")
                print(f"    処理時間: {subject_time:.1f}秒 ({subject_time/60:.1f}分)")
                
                # 結果をCSVファイルに保存
                results_df = pd.DataFrame({
                    'Subject': [subject],
                    'Train_MAE_mmHg': [train_mae],
                    'Train_Corr': [train_corr],
                    'Test_MAE_mmHg': [test_mae],
                    'Test_Corr': [test_corr],
                    'Processing_Time_Sec': [subject_time]
                })
                
                csv_path = subject_save_dir / 'results_summary_SBP.csv'
                ...
                fold_csv_path = subject_save_dir / 'fold_results_SBP.csv'
        ...
        if all_subjects_results:
            ...
            print(f"\n全被験者平均結果 (SBP):")
            print(f"  訓練: MAE={avg_train_mae:.2f}±{std_train_mae:.2f}mmHg, Corr={avg_train_corr:.4f}±{std_train_corr:.4f}")
            print(f"  テスト: MAE={avg_test_mae:.2f}±{std_test_mae:.2f}mmHg, Corr={avg_test_corr:.4f}±{std_test_corr:.4f}")
            ...
            save_dir = Path(config.save_path)
            all_results_df.to_csv(save_dir / 'all_subjects_results_SBP.csv', index=False)
    ...
    # 設定情報も保存
    save_dir = Path(config.save_path)
    with open(save_dir / 'config_summary_SBP.txt', 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("実験設定【SBP予測・Z-score標準化対応版】\n")
        f.write("="*60 + "\n")
        f.write(f"実行日時: {config.timestamp}\n")
        f.write(f"モード: {config.model_mode}\n")
        f.write(f"モデルタイプ: {config.model_type}\n")
        f.write(f"信号タイプ: {config.signal_type} (収縮期血圧)\n")
        f.write(f"信号範囲: {config.signal_range[0]}-{config.signal_range[1]} mmHg\n")
        f.write(f"Z-score標準化: {config.use_zscore_normalization}\n")
        ...
        if config.model_mode == 'cross_subject':
            ...
            f.write(f"Z-score標準化: {'各foldの訓練データから計算' if config.use_zscore_normalization else '未使用'}\n")
            # Cross-Subject結果の詳細を記載
            if os.path.exists(save_dir / 'cross_validation_results_SBP.csv'):
                results_df = pd.read_csv(save_dir / 'cross_validation_results_SBP.csv')
                mean_row = results_df[results_df['Fold'] == 'Mean']
                if not mean_row.empty:
                    f.write(f"平均訓練MAE: {mean_row['Train_MAE_mmHg'].values[0]:.2f} mmHg\n")
                    f.write(f"平均訓練相関: {mean_row['Train_Corr'].values[0]:.4f}\n")
                    f.write(f"平均テストMAE: {mean_row['Test_MAE_mmHg'].values[0]:.2f} mmHg\n")
            ...
            f.write(f"Z-score標準化: {'各被験者・各foldの訓練データから計算' if config.use_zscore_normalization else '未使用'}\n")
            f.write(f"平均訓練MAE: {avg_train_mae:.2f}±{std_train_mae:.2f} mmHg\n")
            f.write(f"平均訓練相関: {avg_train_corr:.4f}±{std_train_corr:.4f}\n")
            f.write(f"平均テストMAE: {avg_test_mae:.2f}±{std_test_mae:.2f} mmHg\n")
        ...
        f.write("\n最適化内容:\n")
        f.write("- Z-score標準化によるSBPの広レンジ対応\n")
    ...
    if config.model_mode == 'cross_subject':
        ...
        print("Z-score標準化: 各foldの訓練データから計算")
    ...
    print(f"\n【Z-score標準化の効果】")
    print(f"  - SBP (60-210 mmHg)の広レンジに対応")
    print(f"  - 訓練データのみから統計量計算（データリーク防止）")
    print(f"  - 学習の安定化（勾配消失・爆発の抑制）")
    print(f"  - 評価時はmmHg単位に逆変換（正確なMAE/RMSE）")
