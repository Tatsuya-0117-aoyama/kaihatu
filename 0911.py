...
class CODataset(Dataset):
    def __init__(self, rgb_data, signal_data, model_type='standard', use_channel='RGB'):
        """
        rgb_data: 
        ...
        """
        # チャンネル選択を適用
        rgb_data = select_channels(rgb_data, use_channel)
...
def task_cross_validation(rgb_data, signal_data, config, subject, subject_save_dir):
    ...
    for fold, test_task in enumerate(config.tasks):
        ...
        # データローダー作成
        train_dataset = CODataset(train_rgb, train_signal, config.model_type, config.use_channel)
        val_dataset = CODataset(val_rgb, val_signal, config.model_type, config.use_channel)
        test_dataset = CODataset(test_rgb, test_signal, config.model_type, config.use_channel)
