...
def select_channels(data, use_channel):
    ...
    データから指定されたチャンネルを選択
    data: (..., 3) 形状のRGBデータ
    use_channel: 'R', 'G', 'B', 'RGB', 'RG', 'GB', 'RB'
    """
    if use_channel == 'R':
        return data[..., 0:1]
    elif use_channel == 'G':
...
class PhysNet2DCNN(nn.Module):
    ...
    def __init__(self, input_shape):
        ...
        in_channels = input_shape[2] if len(input_shape) == 3 else input_shape[-1]
...
# ================================
# CalibrationPhys準拠 PhysNet2DCNN (2D版 - 効率的)
# ================================
...
def load_data_single_subject(subject, config):
    ...
    if config.model_type in ['3d', '2d']:
        ...
        rgb_path = os.path.join(config.rgb_base_path, subject, 
                                f"{subject}_video_data.npy")  # 360フレームのデータ
        if not os.path.exists(rgb_path):
            print(f"警告: {subject}のRGBデータが見つかりません")
            return None, None
        
        rgb_data = np.load(rgb_path)  # Expected shape: (N, 360, 36, 36, 3)
...
def train_model(model, train_loader, val_loader, config, fold=None, subject=None):
    ...
    for epoch in range(config.epochs):
        ...
        for rgb, sig in train_loader:
            ...
            # 予測値と真値を保存
            if pred.dim() == 2 and config.model_type in ['3d', '2d']:
...
def plot_all_subjects_summary(all_subjects_results, config):
    ...
    colors = cm.rainbow(np.linspace(0, 1, len(all_subjects_results)))
    
    # 訓練データ散布図（全被験者を1つのグラフに）
    plt.figure(figsize=(14, 10))
    for i, result in enumerate(all_subjects_results):
        plt.scatter(result['train_corr'], result['train_mae'], 
                   s=100, c=[colors[i]], alpha=0.7, 
                   label=result['subject'], edgecolors='black', linewidth=0.5)
    
    plt.xlabel('相関係数 (Correlation)', fontsize=12)
    plt.ylabel('平均絶対誤差 (MAE)', fontsize=12)
    plt.title(f'全被験者 訓練データ結果\n平均MAE: {np.mean(train_maes):.3f}, 平均Corr: {np.mean(train_corrs):.3f}', 
              fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.axhline(y=np.mean(train_maes), color='r', linestyle='--', alpha=0.5, label=f'平均MAE')
    plt.axvline(x=np.mean(train_corrs), color='b', linestyle='--', alpha=0.5, label=f'平均Corr')
    
    # 凡例を右側に配置
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8, ncol=2)
    plt.tight_layout()
    plt.savefig(save_dir / 'all_subjects_train_scatter.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    # テストデータ散布図（全被験者を1つのグラフに）
    plt.figure(figsize=(14, 10))
    for i, result in enumerate(all_subjects_results):
        plt.scatter(result['test_corr'], result['test_mae'], 
                   s=100, c=[colors[i]], alpha=0.7, 
                   label=result['subject'], edgecolors='black', linewidth=0.5)
    
    plt.xlabel('相関係数 (Correlation)', fontsize=12)
    plt.ylabel('平均絶対誤差 (MAE)', fontsize=12)
    plt.title(f'全被験者 テストデータ結果\n平均MAE: {np.mean(test_maes):.3f}, 平均Corr: {np.mean(test_corrs):.3f}', 
              fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.axhline(y=np.mean(test_maes), color='r', linestyle='--', alpha=0.5, label=f'平均MAE')
    plt.axvline(x=np.mean(test_corrs), color='b', linestyle='--', alpha=0.5, label=f'平均Corr')
    
    # 凡例を右側に配置
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8, ncol=2)
    plt.tight_layout()
    plt.savefig(save_dir / 'all_subjects_test_scatter.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    # MAE比較
    bars1 = ax1.bar(x - width/2, train_maes, width, label='訓練', color='blue', alpha=0.7)
    bars2 = ax1.bar(x + width/2, test_maes, width, label='テスト', color='red', alpha=0.7)
    
    ax1.set_xlabel('被験者', fontsize=12)
    ax1.set_ylabel('MAE', fontsize=12)
    ax1.set_title(f'被験者別MAE比較', fontsize=14)
    ax1.set_xticks(x)
    ax1.set_xticklabels(subjects, rotation=45, ha='right')
    ax1.legend()
    ax1.grid(True, alpha=0.3, axis='y')
    ax1.axhline(y=np.mean(train_maes), color='blue', linestyle='--', alpha=0.5)
    ax1.axhline(y=np.mean(test_maes), color='red', linestyle='--', alpha=0.5)
    
    # 相関係数比較
    bars3 = ax2.bar(x - width/2, train_corrs, width, label='訓練', color='green', alpha=0.7)
    bars4 = ax2.bar(x + width/2, test_corrs, width, label='テスト', color='orange', alpha=0.7)
    
    ax2.set_xlabel('被験者', fontsize=12)
    ax2.set_ylabel('相関係数', fontsize=12)
    ax2.set_title(f'被験者別相関係数比較', fontsize=14)
    ax2.set_xticks(x)
    ax2.set_xticklabels(subjects, rotation=45, ha='right')
    ax2.legend()
    ax2.grid(True, alpha=0.3, axis='y')
    ax2.axhline(y=np.mean(train_corrs), color='green', linestyle='--', alpha=0.5)
    ax2.axhline(y=np.mean(test_corrs), color='orange', linestyle='--', alpha=0.5)
    
    plt.tight_layout()
    plt.savefig(save_dir / 'all_subjects_comparison_bars.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    # 被験者ごとのパフォーマンスランキング（テストデータ基準）
    plt.figure(figsize=(14, 8))
    
    # 相関係数でソート（降順）
    sorted_indices = np.argsort(test_corrs)[::-1]
    sorted_subjects = [subjects[i] for i in sorted_indices]
    sorted_test_corrs = [test_corrs[i] for i in sorted_indices]
    sorted_test_maes = [test_maes[i] for i in sorted_indices]
    ...
    # 相関係数のバー（主軸）
    bars1 = ax1.bar(x, sorted_test_corrs, alpha=0.7, color='blue', label='相関係数')
    ax1.set_xlabel('被験者（相関係数順）', fontsize=12)
    ax1.set_ylabel('相関係数', fontsize=12, color='blue')
    ax1.tick_params(axis='y', labelcolor='blue')
    ax1.set_xticks(x)
    ax1.set_xticklabels(sorted_subjects, rotation=45, ha='right')
    
    # MAEの折れ線（副軸）
    ax2 = ax1.twinx()
    line2 = ax2.plot(x, sorted_test_maes, 'r-o', alpha=0.7, label='MAE')
    ax2.set_ylabel('MAE', fontsize=12, color='red')
    ax2.tick_params(axis='y', labelcolor='red')
    
    # タイトルと凡例
    ax1.set_title('被験者別テストパフォーマンス（相関係数順）', fontsize=14)
    ax1.grid(True, alpha=0.3, axis='y')
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'all_subjects_ranking.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n全被験者プロット作成完了:")
    print(f"  - 訓練/テスト散布図")
    print(f"  - 被験者別比較バープロット")
    print(f"  - パフォーマンスランキング")
