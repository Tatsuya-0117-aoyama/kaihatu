...
class Config:
    def __init__(self):
        ...
        # 推定対象の生体指標選択（新規追加）
        # ================================
        # 選択可能な指標: "CO", "SV", "HR_CO_SV", "Cwk", "Rp", "Zao", "I0", "LVET", "reDIA", "reSYS"
        self.target_signal = "CO"  # ここを変更することで推定対象を切り替え
        
        # 各指標のファイル名プレフィックス設定
        self.signal_prefixes = {
            "CO": "CO_s2",
            "SV": "SV_s2",
            "HR_CO_SV": "HR_CO_SV_s2",
            "Cwk": "Cwk_s2",
            "Rp": "Rp_s2",
            "Zao": "Zao_s2",
            "I0": "I0_s2",
        ...
        # 選択された指標に応じて設定を更新
        self.signal_type = self.target_signal
        self.signal_prefix = self.signal_prefixes.get(self.target_signal, f"{self.target_signal}_s2")
        
        # ================================
        # スライディングウィンドウ設定（3秒ウィンドウに変更）
        # ================================
        self.use_sliding_window = True  # スライディングウィンドウを使用
        self.window_size = 3  # ウィンドウサイズ（秒）※10秒から3秒に変更
        self.window_stride = 3  # ストライド（秒）- 重なりなし
        self.windows_per_task = 20  # 各タスク60秒 / 3秒 = 20ウィンドウ ※6から20に変更
        ...
        # 日付時間フォルダを作成（推定対象を含める）
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.save_path = os.path.join(self.base_save_path, f"{self.timestamp}_{self.target_signal}_3sec")
        ...
        # データ形状設定（3秒スライディングウィンドウ対応）
        if self.use_sliding_window:
            # スライディングウィンドウ使用時は3秒分のフレーム
            self.time_frames = self.window_size
        else:
            # 従来の設定
        ...
        if self.model_type == "3d":
            self.batch_size = 32 if self.use_sliding_window else 8
            self.epochs = 150
            self.learning_rate = 0.001
        elif self.model_type == "2d":
            # 3秒ウィンドウの場合、サンプル数が増えるのでバッチサイズを調整
            self.batch_size = 32 if self.use_sliding_window else (32 if self.use_amp else 16)
            self.epochs = 150
            self.learning_rate = 0.001
        else:
            self.batch_size = 32 if self.use_sliding_window else 16
            self.epochs = 150
            self.learning_rate = 0.001
...
# ================================
# スライディングウィンドウ用データ処理関数（3秒ウィンドウ対応）
# ================================
def create_sliding_windows(data, signal, window_size, stride):
    """
    データを3秒のスライディングウィンドウに分割
    ...
    Returns:
        windows_data: (N_windows, window_size, H, W, C)
        windows_signal: (N_windows, window_size)
    """
    T = data.shape[0]
    ...
    for i in range(n_windows):
        ...
        windows_signal.append(window_signal)
    
    return np.array(windows_data), np.array(windows_signal)
...
# ================================
# CalibrationPhys準拠 PhysNet2DCNN (3D版) - 3秒ウィンドウ対応
# ================================
class PhysNet2DCNN_3D(nn.Module):
    """CalibrationPhys論文準拠のPhysNet2DCNN（3D畳み込み版）- 3秒ウィンドウ対応"""
...
# ================================
# CalibrationPhys準拠 PhysNet2DCNN (2D版) - 3秒ウィンドウ対応
# ================================
class PhysNet2DCNN_2D(nn.Module):
    """2D畳み込みを使用したPhysNet2DCNN（効率的な実装）- 3秒ウィンドウ対応"""
    def __init__(self, input_shape=None):
...
def create_model(config):
    ...
    if config.verbose:
        print(f"\n選択モデル: {model_name}")
        print(f"推定対象指標: {config.target_signal}")
...
# ================================
# データ読み込み関数（3秒スライディングウィンドウ対応）
# ================================
def load_data_single_subject(subject, config):
    """単一被験者のデータを読み込み（LABデータ対応＋複数指標対応）"""
    ...
    for task in config.tasks:
        signal_path = os.path.join(config.signal_base_path, subject, 
                                  config.signal_type,  # 選択された指標のフォルダ
    ...
    print(f"  {config.target_signal}データ読み込み成功: {signal_data.shape}")
...
def load_all_subjects_data(config):
    """全被験者のデータを読み込み（Cross-Subject, Inter-Subject用）- 3秒ウィンドウ版"""
    all_rgb_data = []
    all_signal_data = []
    subject_task_info = []
    ...
    for subject in config.subjects:
        ...
        if config.use_sliding_window:
            ...
            for task_idx, task in enumerate(config.tasks):
                ...
                # 3秒ウィンドウに分割
                windows_rgb, windows_signal = create_sliding_windows(
                    task_rgb, task_signal, 
                    config.window_size, config.window_stride
                )
                
                # 各ウィンドウをサンプルとして追加
                for window_idx in range(len(windows_rgb)):
                    all_rgb_data.append(windows_rgb[window_idx])
                    all_signal_data.append(windows_signal[window_idx])
    ...
    if config.use_sliding_window:
        print(f"  読み込み完了: {len(all_rgb_data)}ウィンドウ分のデータ")
        print(f"  データ形状: RGB={all_rgb_data.shape}, Signal={all_signal_data.shape}")
        print(f"  ウィンドウサイズ: {config.window_size}秒, ストライド: {config.window_stride}秒")
        print(f"  ウィンドウ数/タスク: {config.windows_per_task}")
    else:
...
def train_model(model, train_loader, val_loader, config, fold=None, subject=None):
    ...
    if config.verbose:
        if fold is not None:
            print(f"\n  Fold {fold+1} 学習開始 - 推定対象: {config.target_signal}")
        if subject is not None:
            print(f"\n  {subject} 学習開始 {fold_str} - 推定対象: {config.target_signal}")
    ...
    for epoch in range(config.epochs):
        ...
        if improvement > config.min_delta or val_corr > best_val_corr * config.patience_improvement_threshold:
            ...
            if fold is not None:
                model_name = f'best_model_{config.target_signal}_fold{fold+1}.pth'
            else:
                model_name = f'best_model_{config.target_signal}_{config.model_type}.pth'
        ...
        if config.verbose and ((epoch + 1) % 20 == 0 or epoch == 0 or epoch < config.warmup_epochs):
            ...
            print(f"      推定対象: {config.target_signal}")
...
# ================================
# 被験者別波形プロット関数（3秒スライディングウィンドウ対応）
# ================================
def plot_subject_waveforms_sliding_window(subject_predictions, save_dir, config):
    """被験者別に予測値と真値の波形をプロット（3秒スライディングウィンドウ対応）"""
    
    subjects_dir = save_dir / f"被験者_{config.target_signal}"
    ...
    for subject, data in subject_predictions.items():
        ...
        if config.use_sliding_window:
            # スライディングウィンドウの場合、ウィンドウごとの結果を可視化
            fig, axes = plt.subplots(3, 1, figsize=(16, 12))
            
            # ウィンドウごとにプロット
            n_windows = len(predictions) // config.window_size
            time_points = np.arange(len(predictions))
            
            # 予測値プロット
            axes[0].plot(time_points, predictions, 'b-', linewidth=1.5, label=f'予測値 ({config.target_signal})')
            axes[0].set_ylabel(f'予測値 ({config.target_signal})')
            axes[0].set_title(f'{subject} - 予測値波形 (3秒ウィンドウ)')
            axes[0].grid(True, alpha=0.3)
            axes[0].legend()
            
            # ウィンドウ境界を表示
            for i in range(1, n_windows):
                axes[0].axvline(x=i*config.window_size, color='gray', linestyle='--', alpha=0.3)
            
            # タスク境界を強調表示（20ウィンドウごと）
            for i in range(1, 6):
                axes[0].axvline(x=i*config.windows_per_task*config.window_size, color='red', linestyle='-', alpha=0.5, linewidth=2)
            
            # 真値プロット
            axes[1].plot(time_points, targets, 'g-', linewidth=1.5, label=f'真値 ({config.target_signal})')
            axes[1].set_ylabel(f'真値 ({config.target_signal})')
            axes[1].set_title(f'{subject} - 真値波形 (3秒ウィンドウ)')
            axes[1].grid(True, alpha=0.3)
            axes[1].legend()
            
            # ウィンドウ境界を表示
            for i in range(1, n_windows):
                axes[1].axvline(x=i*config.window_size, color='gray', linestyle='--', alpha=0.3)
            
            # タスク境界を強調表示
            for i in range(1, 6):
                axes[1].axvline(x=i*config.windows_per_task*config.window_size, color='red', linestyle='-', alpha=0.5, linewidth=2)
            
            # 予測値と真値の比較
            axes[2].plot(time_points, targets, 'g-', linewidth=1.5, alpha=0.7, label=f'真値 ({config.target_signal})')
            axes[2].plot(time_points, predictions, 'b-', linewidth=1.5, alpha=0.7, label=f'予測値 ({config.target_signal})')
            axes[2].set_xlabel('時間 (秒)')
            axes[2].set_ylabel(f'信号値 ({config.target_signal})')
            ...
            # ウィンドウ境界を表示
            for i in range(1, n_windows):
                axes[2].axvline(x=i*config.window_size, color='gray', linestyle='--', alpha=0.3)
            
            # タスク境界とタスク名を強調表示
            for i in range(1, 6):
                axes[2].axvline(x=i*config.windows_per_task*config.window_size, color='red', linestyle='-', alpha=0.5, linewidth=2)
                # タスク名を表示
                task_center = (i-0.5)*config.windows_per_task*config.window_size
                axes[2].text(task_center, axes[2].get_ylim()[1]*0.95, 
                           config.tasks[i-1], ha='center', fontsize=8, bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
            
            # 最後のタスクラベル
            task_center = (6-0.5)*config.windows_per_task*config.window_size
            axes[2].text(task_center, axes[2].get_ylim()[1]*0.95, 
                       config.tasks[-1], ha='center', fontsize=8, bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        else:
            # 従来の処理
            fig, axes = plt.subplots(3, 1, figsize=(16, 12))
        ...
        plt.tight_layout()
        plt.savefig(subject_dir / f'waveform_comparison_{config.target_signal}_3sec.png', dpi=150, bbox_inches='tight')
        plt.close()
        ...
        plt.ylabel(f'予測値 ({config.target_signal})')
        plt.title(f'{subject} - 散布図 (相関係数: {corr:.4f})')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(subject_dir / f'scatter_plot_{config.target_signal}_3sec.png', dpi=150, bbox_inches='tight')
        plt.close()
    
    return subjects_dir

def save_correlation_ranking(subject_correlations, save_dir, config):
    ...
    # ランキングファイルの作成
    ranking_file = subjects_dir / f"相関係数ランキング_{config.target_signal}_3sec.txt"
    with open(ranking_file, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write(f"被験者別相関係数ランキング - {config.target_signal}推定 (3秒ウィンドウ)\n")
        f.write("="*60 + "\n\n")
        ...
        f.write(f"ウィンドウサイズ: {config.window_size}秒\n")
    ...
    # 上位3名と下位3名を表示
    print(f"\n【{config.target_signal} 相関係数ランキング (3秒ウィンドウ)】")
    print("上位3名:")
...
# ================================
# Inter-Subject 5分割交差検証（3秒スライディングウィンドウ対応）
# ================================
def inter_subject_cv(config):
    """個人間5分割交差検証（被験者レベルでの分割＋3秒スライディングウィンドウ対応）"""
    
    print(f"\n推定対象指標: {config.target_signal}")
    if config.use_sliding_window:
        print(f"スライディングウィンドウ: 有効（{config.window_size}秒ウィンドウ、ストライド{config.window_stride}秒）")
        print(f"各タスクあたりのウィンドウ数: {config.windows_per_task}")
    ...
    # データ分割情報を保存
    split_info_file = save_dir / f'inter_subject_split_info_{config.target_signal}_3sec.txt'
    with open(split_info_file, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write(f"Inter-Subject 5分割交差検証 データ分割情報 - {config.target_signal}推定 (3秒ウィンドウ)\n")
        ...
        if config.use_sliding_window:
            f.write(f"総ウィンドウ数/被験者: {len(config.tasks) * config.windows_per_task}\n")
        f.write("\n")
    ...
    for fold_idx in range(config.n_folds):
        print(f"\n{'='*60}")
        print(f"Fold {fold_idx + 1}/{config.n_folds} - {config.target_signal}推定 (3秒ウィンドウ)")
        print(f"{'='*60}")
        ...
        print(f"  データサイズ:")
        if config.use_sliding_window:
            print(f"    訓練: {len(train_rgb)}ウィンドウ (3秒ウィンドウ)")
            print(f"    検証: {len(val_rgb)}ウィンドウ (3秒ウィンドウ)")
            print(f"    テスト: {len(test_rgb)}ウィンドウ (3秒ウィンドウ)")
        else:
        ...
        print(f"\n  Fold {fold_idx + 1} 結果 ({config.target_signal}, 3秒ウィンドウ):")
    ...
    print(f"\n{'='*60}")
    print(f"Inter-Subject 5分割交差検証 最終結果 - {config.target_signal}推定（3秒ウィンドウ、キャリブレーション適用後）")
    print(f"{'='*60}")
    ...
    results_df.to_csv(save_dir / f'inter_subject_results_{config.target_signal}_3sec_calibrated.csv', index=False)
    ...
    subject_results_df.to_csv(subjects_dir / f'subject_correlations_{config.target_signal}_3sec.csv', index=False)
    
    return fold_results

# ================================
# プロット関数（Inter-Subject用 - 複数指標対応）
# ================================
def plot_fold_results_inter_subject(result, fold_dir, config):
    ...
    plt.title(f"Fold {fold} 訓練データ - {config.target_signal}推定{window_info}\n" +
             f"MAE: {result['train_mae']:.3f}, Corr: {result['train_corr']:.3f}\n" +
             f"訓練被験者: {len(result['train_subjects'])}人")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(fold_dir / f'train_scatter_{config.target_signal}_3sec_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.title(f"Fold {fold} テストデータ - {config.target_signal}推定{window_info}\n" +
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(fold_dir / f'test_scatter_{config.target_signal}_3sec_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()

def plot_all_folds_summary_inter_subject(fold_results, save_dir, config):
    """全Foldの結果を統合してプロット（Inter-Subject版）"""
    
    window_info = " (3秒ウィンドウ)" if config.use_sliding_window else ""
    ...
    avg_train_mae = np.mean([r['train_mae'] for r in fold_results])
    avg_train_corr = np.mean([r['train_corr'] for r in fold_results])
    plt.title(f'Inter-Subject 全Fold 訓練データ - {config.target_signal}推定{window_info}\n' +
             f'平均MAE: {avg_train_mae:.3f}, 平均Corr: {avg_train_corr:.3f}')
    
    plt.legend(loc='upper left')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_dir / f'all_folds_train_scatter_{config.target_signal}_3sec_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    avg_test_mae = np.mean([r['test_mae'] for r in fold_results])
    avg_test_corr = np.mean([r['test_corr'] for r in fold_results])
    plt.title(f'Inter-Subject 全Fold テストデータ - {config.target_signal}推定{window_info}\n' +
             f'平均MAE: {avg_test_mae:.3f}, 平均Corr: {avg_test_corr:.3f}')
    
    plt.legend(loc='upper left')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_dir / f'all_folds_test_scatter_{config.target_signal}_3sec_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    ax1.set_title(f'Inter-Subject 各FoldのMAE - {config.target_signal}推定{window_info}（キャリブレーション後）')
    ax1.legend()
    ...
    ax2.set_ylabel('相関係数')
    ax2.set_title(f'Inter-Subject 各Foldの相関係数 - {config.target_signal}推定{window_info}（キャリブレーション後）')
    ax2.set_ylim([0, 1])
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(save_dir / f'fold_performance_comparison_{config.target_signal}_3sec_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()

# ================================
# 複数指標の統合解析関数（3秒ウィンドウ対応）
# ================================
def run_multiple_signals_analysis(config, signals_to_analyze):
    """複数の生体指標について順次解析を実行"""
    
    all_results = {}
    
    print("\n" + "="*60)
    print("複数指標統合解析開始 (3秒ウィンドウ)")
    print("="*60)
    print(f"解析対象指標: {', '.join(signals_to_analyze)}")
    print(f"解析モード: {config.model_mode}")
    print(f"ウィンドウサイズ: {config.window_size}秒")
    print(f"ストライド: {config.window_stride}秒")
    ...
    for signal in signals_to_analyze:
        ...
        print(f"\n{'='*60}")
        print(f"{signal} 推定モデル構築開始 (3秒ウィンドウ)")
        print(f"{'='*60}")
        
        # 指標を切り替え
        config.target_signal = signal
        config.signal_type = signal
        config.signal_prefix = config.signal_prefixes.get(signal, f"{signal}_s2")
        
        # タイムスタンプを更新して新しい保存先を作成
        config.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        config.save_path = os.path.join(config.base_save_path, f"{config.timestamp}_{signal}_3sec")
        
        # モードに応じた解析を実行
        if config.model_mode == 'inter_subject':
            results = inter_subject_cv(config)
        elif config.model_mode == 'cross_subject':
            results = cross_subject_cv(config)
        else:  # within_subject
            print(f"{signal}のWithin-Subject解析は現在未実装です")
            continue
        ...
        print(f"\n{signal} 解析完了 (3秒ウィンドウ)")
        print(f"処理時間: {signal_time:.1f}秒 ({signal_time/60:.1f}分)")
    
    # 全指標の結果を統合してサマリー作成
    create_multi_signal_summary(all_results, config)
    
    total_time = time.time() - total_start
    print(f"\n{'='*60}")
    print("全指標解析完了 (3秒ウィンドウ)")
    print(f"総処理時間: {total_time:.1f}秒 ({total_time/60:.1f}分)")
    print(f"{'='*60}")
    
    return all_results

def create_multi_signal_summary(all_results, config):
    """複数指標の結果を統合してサマリーを作成（3秒ウィンドウ版）"""
    
    save_dir = Path(config.base_save_path) / f"summary_{config.timestamp}_3sec"
    save_dir.mkdir(parents=True, exist_ok=True)
    
    # 各指標の平均性能を集計
    summary_data = []
    for signal, fold_results in all_results.items():
        avg_train_mae = np.mean([r['train_mae'] for r in fold_results])
        avg_train_corr = np.mean([r['train_corr'] for r in fold_results])
        avg_test_mae = np.mean([r['test_mae'] for r in fold_results])
        avg_test_corr = np.mean([r['test_corr'] for r in fold_results])
        ...
        summary_data.append({
            ...
            'Test_Corr_Mean': avg_test_corr,
            'Test_Corr_Std': std_test_corr
        })
    
    # DataFrameに変換してCSV保存
    summary_df = pd.DataFrame(summary_data)
    summary_df = summary_df.sort_values('Test_Corr_Mean', ascending=False)
    summary_df.to_csv(save_dir / 'multi_signal_summary_3sec.csv', index=False)
    
    # 比較プロットの作成
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    
    # MAE比較
    signals = summary_df['Signal'].tolist()
    mae_means = summary_df['Test_MAE_Mean'].tolist()
    mae_stds = summary_df['Test_MAE_Std'].tolist()
    
    ax1.bar(signals, mae_means, yerr=mae_stds, capsize=5)
    ax1.set_xlabel('推定対象指標')
    ax1.set_ylabel('テストMAE')
    ax1.set_title(f'各指標のMAE比較 (3秒ウィンドウ)')
    ax1.tick_params(axis='x', rotation=45)
    ax1.grid(True, alpha=0.3)
    
    # 相関係数比較
    corr_means = summary_df['Test_Corr_Mean'].tolist()
    corr_stds = summary_df['Test_Corr_Std'].tolist()
    
    ax2.bar(signals, corr_means, yerr=corr_stds, capsize=5)
    ax2.set_xlabel('推定対象指標')
    ax2.set_ylabel('テスト相関係数')
    ax2.set_title(f'各指標の相関係数比較 (3秒ウィンドウ)')
    ax2.set_ylim([0, 1])
    ax2.tick_params(axis='x', rotation=45)
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(save_dir / 'multi_signal_comparison_3sec.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n複数指標サマリーを保存: {save_dir}")
    print(f"\n【推定性能ランキング（相関係数順）- 3秒ウィンドウ】")
    for idx, row in summary_df.iterrows():
        print(f"{row['Signal']}: Corr={row['Test_Corr_Mean']:.3f}±{row['Test_Corr_Std']:.3f}, " +
              f"MAE={row['Test_MAE_Mean']:.3f}±{row['Test_MAE_Std']:.3f}")

# ================================
# Cross-Subject 5分割交差検証（3秒スライディングウィンドウ対応）
# ================================
def cross_subject_cv(config):
    """被験者間5分割交差検証（キャリブレーション版 + 3秒スライディングウィンドウ対応）"""
    ...
    # データ分割情報を保存
    split_info_file = save_dir / f'data_split_info_{config.target_signal}_3sec.txt'
    with open(split_info_file, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write(f"Cross-Subject 5分割交差検証 データ分割情報 - {config.target_signal}推定 (3秒ウィンドウ)\n")
    ...
    print(f"\n{'='*60}")
    print(f"5分割交差検証 最終結果 - {config.target_signal}推定（3秒ウィンドウ、キャリブレーション適用後）")
    print(f"{'='*60}")
    print(f"訓練平均: MAE={avg_train_mae:.4f}, Corr={avg_train_corr:.4f}")
    print(f"テスト平均: MAE={avg_test_mae:.4f}±{std_test_mae:.4f}, Corr={avg_test_corr:.4f}±{std_test_corr:.4f}")
    ...
    results_df = pd.concat([results_df, mean_row, std_row], ignore_index=True)
    results_df.to_csv(save_dir / f'cross_validation_results_{config.target_signal}_3sec_calibrated.csv', index=False)
...
def plot_all_folds_summary_cross_subject(fold_results, save_dir, config):
    """全Foldの結果を統合してプロット（Cross-Subject版）"""
    ...
    avg_train_mae = np.mean([r['train_mae'] for r in fold_results])
    avg_train_corr = np.mean([r['train_corr'] for r in fold_results])
    plt.title(f'全Fold 訓練データ - {config.target_signal}推定{window_info}\n' +
             f'平均MAE: {avg_train_mae:.3f}, 平均Corr: {avg_train_corr:.3f}')
    ...
    avg_test_mae = np.mean([r['test_mae'] for r in fold_results])
    avg_test_corr = np.mean([r['test_corr'] for r in fold_results])
    plt.title(f'全Fold テストデータ - {config.target_signal}推定{window_info}\n' +
             f'平均MAE: {avg_test_mae:.3f}, 平均Corr: {avg_test_corr:.3f}')
    ...
    ax1.axhline(y=np.mean(mae_values), color='r', linestyle='--', 
                label=f'平均: {np.mean(mae_values):.3f}')
    ax1.set_ylabel(f'MAE ({config.target_signal})')
    ax1.set_title(f'各FoldのMAE - {config.target_signal}推定{window_info}（キャリブレーション後）')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ...
    ax2.axhline(y=np.mean(corr_values), color='r', linestyle='--', 
                label=f'平均: {np.mean(corr_values):.3f}')
    ax2.set_ylabel('相関係数')
    ax2.set_title(f'各Foldの相関係数 - {config.target_signal}推定{window_info}（キャリブレーション後）')
    ax2.set_ylim([0, 1])
    ax2.legend()
    ax2.grid(True, alpha=0.3)
...
def plot_subject_summary_colored(fold_results, all_test_predictions, all_test_targets,
    ...
    if all_train_predictions:
        ...
        plt.xlabel(f'真値 ({config.target_signal})')
        plt.ylabel(f'予測値 ({config.target_signal})（キャリブレーション後）')
        plt.title(f"{subject} 全訓練データ - {config.target_signal}推定\n" +
                 f"MAE: {all_train_mae:.3f}, Corr: {all_train_corr:.3f}")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(subject_save_dir / f'all_train_scatter_{config.target_signal}_calibrated.png', dpi=150, bbox_inches='tight')
        plt.close()
    ...
    plt.xlabel(f'真値 ({config.target_signal})')
    plt.ylabel(f'予測値 ({config.target_signal})（キャリブレーション後）')
    plt.title(f"{subject} 全テストデータ - {config.target_signal}推定\n" +
             f"MAE: {all_test_mae:.3f}, Corr: {all_test_corr:.3f}")
    plt.legend(loc='upper left', bbox_to_anchor=(1.02, 1))
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(subject_save_dir / f'all_test_scatter_colored_{config.target_signal}_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    # 波形プロット
    plt.figure(figsize=(16, 10))
    time_points = np.arange(len(all_test_predictions))
    
    # サブプロット1: 予測値
    plt.subplot(3, 1, 1)
    plt.plot(time_points, all_test_predictions, 'b-', linewidth=1.5, label=f'予測値 ({config.target_signal})')
    plt.ylabel(f'予測値 ({config.target_signal})')
    plt.title(f'{subject} - タスクごとの予測波形')
    plt.grid(True, alpha=0.3)
    plt.legend()
    ...
    for i in range(1, 6):
        ...
        plt.text((i-0.5)*60, plt.ylim()[1]*0.95, config.tasks[i-1], ha='center', fontsize=8)
    plt.text(330, plt.ylim()[1]*0.95, config.tasks[-1], ha='center', fontsize=8)
    
    # サブプロット2: 真値
    plt.subplot(3, 1, 2)
    plt.plot(time_points, all_test_targets, 'g-', linewidth=1.5, label=f'真値 ({config.target_signal})')
    plt.ylabel(f'真値 ({config.target_signal})')
    plt.title(f'{subject} - タスクごとの真値波形')
    plt.grid(True, alpha=0.3)
    plt.legend()
    ...
    # サブプロット3: 比較
    plt.subplot(3, 1, 3)
    plt.plot(time_points, all_test_targets, 'g-', linewidth=1.5, alpha=0.7, label=f'真値 ({config.target_signal})')
    plt.plot(time_points, all_test_predictions, 'b-', linewidth=1.5, alpha=0.7, label=f'予測値 ({config.target_signal})')
    plt.xlabel('時間 (秒)')
    plt.ylabel(f'信号値 ({config.target_signal})')
    plt.title(f'{subject} - 予測値と真値の比較 (相関係数: {all_test_corr:.4f})')
    plt.grid(True, alpha=0.3)
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig(subject_save_dir / f'waveform_comparison_{config.target_signal}_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    return all_train_mae, all_train_corr, all_test_mae, all_test_corr

# ================================
# Within-Subject 6分割交差検証（3秒スライディングウィンドウ対応 - 完全版）
# ================================
def task_cross_validation_sliding_window(rgb_data, signal_data, config, subject, subject_save_dir):
    """タスクごとの6分割交差検証（スライディングウィンドウ対応）"""
    ...
    if config.use_sliding_window:
        ...
        for task_idx, task in enumerate(config.tasks):
            ...
            all_windows_rgb.extend(windows_rgb)
            all_windows_signal.extend(windows_signal)
            task_labels.extend([task] * len(windows_rgb))
        
        all_windows_rgb = np.array(all_windows_rgb)
        all_windows_signal = np.array(all_windows_signal)
        
        # ウィンドウベースの交差検証
        fold_results = []
        all_test_predictions = []
        all_test_targets = []
        all_test_window_indices = []
        all_test_tasks = []
        calibration_params_all = []
        ...
        # タスクごとにテストデータとする6分割交差検証
        for fold, test_task in enumerate(config.tasks):
            fold_start_time = time.time()
            
            if config.verbose:
                print(f"\n  Fold {fold+1}/6 - テストタスク: {test_task} ({config.target_signal}推定, 3秒ウィンドウ)")
            
            train_indices = []
            test_indices = []
            
            for i, task_label in enumerate(task_labels):
                if task_label == test_task:
                    test_indices.append(i)
                else:
                    train_indices.append(i)
            ...
            train_rgb = all_windows_rgb[train_indices]
            train_signal = all_windows_signal[train_indices]
            val_rgb = all_windows_rgb[val_indices]
            val_signal = all_windows_signal[val_indices]
            test_rgb = all_windows_rgb[test_indices]
            test_signal = all_windows_signal[test_indices]
            
            if config.verbose:
                print(f"    データサイズ - 訓練: {len(train_rgb)}ウィンドウ, " +
                      f"検証: {len(val_rgb)}ウィンドウ, テスト: {len(test_rgb)}ウィンドウ (各3秒)")
            
            # データローダー作成
            train_dataset = CODataset(train_rgb, train_signal,
                                     config.model_type, config.use_channel, config, is_training=True)
            val_dataset = CODataset(val_rgb, val_signal,
                                   config.model_type, config.use_channel, config, is_training=False)
            test_dataset = CODataset(test_rgb, test_signal,
                                    config.model_type, config.use_channel, config, is_training=False)
            ...
            all_test_window_indices.extend(test_indices)
            all_test_tasks.extend([test_task] * len(test_indices))
            
            # 各Foldのプロット
            plot_fold_results_colored_sliding_window(fold_results[-1], subject_save_dir, config)
        ...
        # ウィンドウを元の順序に並び替え
        sorted_indices = np.argsort(all_test_window_indices)
        ...
        # 従来の処理（元のtask_cross_validation関数と同じ）
        fold_results = []
        all_test_predictions = []
        all_test_targets = []
        ...
        for fold, test_task in enumerate(config.tasks):
            ...
            if config.verbose:
                print(f"\n  Fold {fold+1}/6 - テストタスク: {test_task} ({config.target_signal}推定)")
                print(f"    検証データ分割戦略: {config.validation_split_strategy}")
    ...
    calib_df.to_csv(subject_save_dir / f'calibration_parameters_{config.target_signal}_3sec.csv', index=False)
    
    return fold_results, all_test_predictions, all_test_targets, all_test_tasks

# ================================
# Within-Subjectメイン処理（3秒ウィンドウ対応）
# ================================
def within_subject_analysis(config):
    """Within-Subject解析のメイン処理（複数指標対応）"""
    
    print(f"\n{'='*60}")
    print(f"Within-Subject モード: 個人内6分割交差検証")
    print(f"推定対象: {config.target_signal}")
    if config.use_sliding_window:
        print(f"スライディングウィンドウ: 有効（{config.window_size}秒）")
    if config.use_calibration:
    ...
    for subj_idx, subject in enumerate(config.subjects):
        ...
        print(f"\n{'='*60}")
        print(f"被験者 {subject} ({subj_idx+1}/{len(config.subjects)}) - {config.target_signal}推定")
        print(f"{'='*60}")
        ...
        try:
            ...
            # 6分割交差検証実行（スライディングウィンドウ対応版）
            fold_results, all_test_predictions, all_test_targets, all_test_tasks = task_cross_validation_sliding_window(
                rgb_data, signal_data, config, subject, subject_save_dir
            )
            
            # 被験者全体のサマリープロット
            if config.use_sliding_window:
                train_mae, train_corr, test_mae, test_corr = plot_subject_summary_colored_sliding_window(
            ...
            print(f"\n  {subject} 完了 ({config.target_signal}推定、キャリブレーション後):")
            ...
            results_df = pd.DataFrame({
                'Subject': [subject],
                'Target_Signal': [config.target_signal],
                'Window_Size_Sec': [config.window_size if config.use_sliding_window else 'N/A'],
                'Train_MAE': [train_mae],
                'Train_Corr': [train_corr],
                'Test_MAE': [test_mae],
                'Test_Corr': [test_corr],
                ...
                'Sliding_Window': ['Yes' if config.use_sliding_window else 'No']
            })
            
            csv_path = subject_save_dir / f'results_summary_{config.target_signal}_3sec_calibrated.csv'
            ...
            fold_csv_path = subject_save_dir / f'fold_results_{config.target_signal}_3sec_calibrated.csv'
    ...
    if all_subjects_results:
        print(f"\n{'='*60}")
        print(f"全被験者サマリー作成中 ({config.target_signal})...")
        ...
        print(f"\n全被験者平均結果 ({config.target_signal}推定、3秒ウィンドウ、キャリブレーション後):")
        ...
        save_dir = Path(config.save_path)
        all_results_df.to_csv(save_dir / f'all_subjects_results_{config.target_signal}_3sec_calibrated.csv', index=False)
...
def main():
    ...
    print("\n" + "="*60)
    print(" PhysNet2DCNN - 生体指標推定（3秒スライディングウィンドウ + 複数指標対応）")
    print("="*60)
    print(f"モード: {config.model_mode}")
    ...
    # 複数指標を順次解析するかどうかの設定
    analyze_multiple_signals = False  # Trueに設定すると複数指標を解析
    
    if analyze_multiple_signals:
        # 解析したい指標のリスト（順番に実行）
        signals_to_analyze = ["CO", "SV", "HR_CO_SV", "Cwk", "Rp", "Zao", "I0", "LVET", "reDIA", "reSYS"]
        # またはテスト用に一部のみ
        # signals_to_analyze = ["CO", "SV", "HR_CO_SV"]
        
        print(f"\n複数指標解析モード")
        print(f"解析対象: {', '.join(signals_to_analyze)}")
        
        # 複数指標の統合解析を実行
        all_results = run_multiple_signals_analysis(config, signals_to_analyze)
        ...
        if config.use_sliding_window:
            ...
            print(f"  ストライド: {config.window_stride}秒")
            print(f"  ウィンドウ数/タスク: {config.windows_per_task}")
            print(f"  総ウィンドウ数/被験者: {config.windows_per_task * len(config.tasks)}")
        else:
            print(f"  状態: 無効（従来の60秒/360秒処理）")
        ...
        if config.model_mode == 'inter_subject':
            ...
            print("Within-Subjectモードは現在未実装です")
    ...
    with open(save_dir / 'config_summary.txt', 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("実験設定【3秒スライディングウィンドウ + 複数指標対応版】\n")
        f.write("="*60 + "\n")
        ...
        if analyze_multiple_signals:
            f.write(f"解析モード: 複数指標統合解析\n")
            f.write(f"解析対象指標: {', '.join(signals_to_analyze)}\n")
        else:
            f.write(f"解析モード: 単一指標解析\n")
            f.write(f"推定対象指標: {config.target_signal}\n")
        ...
        if config.use_sliding_window:
            f.write(f"\n3秒スライディングウィンドウ:\n")
            f.write(f"  ウィンドウサイズ: {config.window_size}秒\n")
            f.write(f"  ストライド: {config.window_stride}秒\n")
            f.write(f"  ウィンドウ数/タスク: {config.windows_per_task}\n")
            f.write(f"  総ウィンドウ数/被験者: {config.windows_per_task * len(config.tasks)}\n")
        
        f.write(f"\n線形キャリブレーション: {config.use_calibration}\n")
        ...
        if config.model_mode == 'within_subject' and not analyze_multiple_signals:
            f.write(f"平均処理時間/被験者: 未実装\n")
    ...
    print(f"\n{'='*60}")
    print("処理完了")
    print(f"結果保存先: {config.save_path if not analyze_multiple_signals else config.base_save_path}")
    print(f"総処理時間: {total_time:.1f}秒 ({total_time/60:.1f}分)")
    
    if config.use_sliding_window:
        print("\n【3秒スライディングウィンドウ】")
        print(f"  3秒ウィンドウで学習・評価を実施")
        print(f"  各タスク60秒を{config.windows_per_task}ウィンドウに分割")
        print(f"  総ウィンドウ数/被験者: {config.windows_per_task * len(config.tasks)}")
    
    if analyze_multiple_signals:
        print("\n【複数指標解析完了】")
        print(f"  解析指標数: {len(signals_to_analyze)}")
        print(f"  平均処理時間/指標: {total_time/len(signals_to_analyze):.1f}秒")
