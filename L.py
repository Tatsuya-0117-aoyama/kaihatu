...
if __name__ == "__main__":
    main()
前処理済みのRGB信号（bpXXX_downsampled_1Hz.npy：形状 (N, 14, 16, C)）と、各タスク60秒のCOラベル（CO_s2_t*.npyを6つ）を既存パスに置けば動きます。

事前学習は 32人×各360秒=11,520サンプル前提で、バッチを 16人×各8枚=128 に設計（Configで変更可）。

まずはデフォルトのまま走らせ、GPUメモリに応じて subjects_per_batch / samples_per_subject / pretrain_epochs を調整してください。

被験者の個体差（性別・年齢・顔形状・画角・肌色）にロバスト化する主なポイントは

時間的ポジティブ、2) 同一被験者を負例から除外、3) 逆学習でID情報を抑圧 です。

ファインチューニングは 最初のfreeze_backbone_epochsエポックはBackbone凍結 → その後全層学習。必要に応じて 0〜10 に調整してください。
