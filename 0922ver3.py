...
# ================================
# 設定クラス（モード切り替え対応＋スライディングウィンドウ対応）
# ================================
class Config:
    def __init__(self):
        # ================================
        ...
        self.window_size = 10  # ウィンドウサイズ（秒）
        self.window_stride = 10  # ストライド（秒）- 重なりなし
        self.windows_per_task = 6  # 各タスク60秒 / 10秒 = 6ウィンドウ
        ...
        self.save_path = os.path.join(self.base_save_path, f"{self.timestamp}_{self.target_signal}")
        ...
        # データ形状設定（スライディングウィンドウ対応）
        if self.use_sliding_window:
            # スライディングウィンドウ使用時は10秒分のフレーム
        ...
        if self.model_type == "3d":
            self.batch_size = 16 if self.use_sliding_window else 8
            self.epochs = 150
            self.learning_rate = 0.001
        elif self.model_type == "2d":
            self.batch_size = 64 if self.use_sliding_window else (32 if self.use_amp else 16)
            self.epochs = 150
            self.learning_rate = 0.001
        else:
...
# ================================
# CalibrationPhys準拠 PhysNet2DCNN (3D版) - スライディングウィンドウ対応
# ================================
...
# ================================
# CalibrationPhys準拠 PhysNet2DCNN (2D版) - スライディングウィンドウ対応
# ================================
...
def plot_subject_waveforms_sliding_window(subject_predictions, save_dir, config):
    ...
    for subject, data in subject_predictions.items():
        ...
        if config.use_sliding_window:
            ...
            axes[0].set_title(f'{subject} - 予測値波形 (10秒ウィンドウ)')
            ...
            axes[1].set_title(f'{subject} - 真値波形 (10秒ウィンドウ)')
        ...
        plt.tight_layout()
        plt.savefig(subject_dir / f'waveform_comparison_{config.target_signal}.png', dpi=150, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(subject_dir / f'scatter_plot_{config.target_signal}.png', dpi=150, bbox_inches='tight')
        plt.close()
...
def save_correlation_ranking(subject_correlations, save_dir, config):
    ...
    # ランキングファイルの作成
    ranking_file = subjects_dir / f"相関係数ランキング_{config.target_signal}.txt"
    with open(ranking_file, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write(f"被験者別相関係数ランキング - {config.target_signal}推定\n")
        f.write("="*60 + "\n\n")
    ...
    # 上位3名と下位3名を表示
    print(f"\n【{config.target_signal} 相関係数ランキング】")
    print("上位3名:")
...
# ================================
# Inter-Subject 5分割交差検証（スライディングウィンドウ対応）
# ================================
def inter_subject_cv(config):
    """個人間5分割交差検証（被験者レベルでの分割＋スライディングウィンドウ対応）"""
    ...
    # データ分割情報を保存
    split_info_file = save_dir / f'inter_subject_split_info_{config.target_signal}.txt'
    with open(split_info_file, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write(f"Inter-Subject 5分割交差検証 データ分割情報 - {config.target_signal}推定\n")
    ...
    print(f"\n{'='*60}")
    print(f"Inter-Subject 5分割交差検証 最終結果 - {config.target_signal}推定（キャリブレーション適用後）")
    print(f"{'='*60}")
    ...
    results_df.to_csv(save_dir / f'inter_subject_results_{config.target_signal}_calibrated.csv', index=False)
...
def plot_fold_results_inter_subject(result, fold_dir, config):
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(fold_dir / f'train_scatter_{config.target_signal}_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()
...
def task_cross_validation_sliding_window(rgb_data, signal_data, config, subject, subject_save_dir):
        ...
        for fold, test_task in enumerate(config.tasks):
            ...
            if config.verbose:
                print(f"    データサイズ - 訓練: {len(train_rgb)}ウィンドウ, " +
                      f"検証: {len(val_rgb)}ウィンドウ, テスト: {len(test_rgb)}ウィンドウ")
    ...
    calib_df.to_csv(subject_save_dir / f'calibration_parameters_{config.target_signal}.csv', index=False)
...
def plot_fold_results_colored_sliding_window(result, save_dir, config):
    ...
    if result['train_predictions'] is not None and result['train_targets'] is not None:
        ...
        plt.xlabel(f'真値 ({config.target_signal})')
        plt.ylabel(f'予測値 ({config.target_signal})（キャリブレーション後）')
        plt.title(f"Fold {fold} 訓練データ - {config.target_signal}推定 (10秒ウィンドウ)\n" +
                 f"MAE: {result['train_mae']:.3f}, Corr: {result['train_corr']:.3f}")
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.tight_layout()
    ...
    test_targets_flat = result['test_targets'].flatten()
    plt.scatter(test_targets_flat, test_preds_flat,
                alpha=0.6, s=20, color=task_color, label=f'テストタスク: {test_task} (10秒ウィンドウ)')
    min_val = min(test_targets_flat.min(), test_preds_flat.min())
    max_val = max(test_targets_flat.max(), test_preds_flat.max())
    plt.plot([min_val, max_val], [min_val, max_val], 'r--', lw=2)
    plt.xlabel(f'真値 ({config.target_signal})')
    plt.ylabel(f'予測値 ({config.target_signal})（キャリブレーション後）')
    plt.title(f"Fold {fold} テスト ({test_task}) - {config.target_signal}推定 (10秒ウィンドウ)\n" +
             f"MAE: {result['test_mae']:.3f}, Corr: {result['test_corr']:.3f}\n" +
             f"キャリブレーション: y = {result['calibration_a']:.3f} * ŷ + {result['calibration_b']:.3f}")
    plt.grid(True, alpha=0.3)
    plt.legend()
...
def plot_subject_summary_colored_sliding_window(fold_results, all_test_predictions, all_test_targets,
    ...
    if all_train_predictions:
        ...
        plt.xlabel(f'真値 ({config.target_signal})')
        plt.ylabel(f'予測値 ({config.target_signal})（キャリブレーション後）')
        plt.title(f"{subject} 全訓練データ - {config.target_signal}推定 (10秒ウィンドウ)\n" +
    ...
    plt.xlabel(f'真値 ({config.target_signal})')
    plt.ylabel(f'予測値 ({config.target_signal})（キャリブレーション後）')
    plt.title(f"{subject} 全テストデータ - {config.target_signal}推定 (10秒ウィンドウ)\n" +
...
def plot_all_subjects_summary_unified(all_subjects_results, config):
    ...
    ax1.set_ylabel('相関係数')
    ax1.set_title(f'訓練データ相関 - {config.target_signal}推定（キャリブレーション後）')
    ax1.set_ylim([0, 1])
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ...
    ax2.set_ylabel('相関係数')
    ax2.set_xlabel('被験者')
    ax2.set_title(f'テストデータ相関 - {config.target_signal}推定（キャリブレーション後）')
    ax2.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(save_dir / f'all_subjects_performance_comparison_{config.target_signal}_calibrated.png', dpi=150, bbox_inches='tight')
    plt.close()
...
def within_subject_analysis(config):
    ...
    for subj_idx, subject in enumerate(config.subjects):
        ...
        try:
            ...
            csv_path = subject_save_dir / f'results_summary_{config.target_signal}_calibrated.csv'
            ...
            fold_csv_path = subject_save_dir / f'fold_results_{config.target_signal}_calibrated.csv'
    ...
    if all_subjects_results:
        ...
        print(f"\n全被験者平均結果 ({config.target_signal}推定、キャリブレーション後):")
        ...
        save_dir = Path(config.save_path)
        all_results_df.to_csv(save_dir / f'all_subjects_results_{config.target_signal}_calibrated.csv', index=False)
...
def plot_all_folds_summary_cross_subject(fold_results, save_dir, config):
    ...
    avg_train_mae = np.mean([r['train_mae'] for r in fold_results])
    avg_train_corr = np.mean([r['train_corr'] for r in fold_results])
    plt.title(f'全Fold 訓練データ - {config.target_signal}推定\n' +
             f'平均MAE: {avg_train_mae:.3f}, 平均Corr: {avg_train_corr:.3f}')
    ...
    avg_test_mae = np.mean([r['test_mae'] for r in fold_results])
    avg_test_corr = np.mean([r['test_corr'] for r in fold_results])
    plt.title(f'全Fold テストデータ - {config.target_signal}推定\n' +
             f'平均MAE: {avg_test_mae:.3f}, 平均Corr: {avg_test_corr:.3f}')
    ...
    ax1.axhline(y=np.mean(mae_values), color='r', linestyle='--', 
                label=f'平均: {np.mean(mae_values):.3f}')
    ax1.set_ylabel(f'MAE ({config.target_signal})')
    ax1.set_title(f'各FoldのMAE - {config.target_signal}推定（キャリブレーション後）')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ...
    ax2.axhline(y=np.mean(corr_values), color='r', linestyle='--', 
                label=f'平均: {np.mean(corr_values):.3f}')
    ax2.set_ylabel('相関係数')
    ax2.set_title(f'各Foldの相関係数 - {config.target_signal}推定（キャリブレーション後）')
    ax2.set_ylim([0, 1])
    ax2.legend()
    ax2.grid(True, alpha=0.3)
...
def run_multiple_signals_analysis(config, signals_to_analyze):
    ...
    for signal in signals_to_analyze:
        ...
        config.save_path = os.path.join(config.base_save_path, f"{config.timestamp}_{signal}")
        ...
        if config.model_mode == 'inter_subject':
            ...
            print(f"{signal}のWithin-Subject解析は未実装です")
            continue
...
def create_multi_signal_summary(all_results, config):
    ...
    save_dir = Path(config.base_save_path) / f"summary_{config.timestamp}"
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    summary_df.to_csv(save_dir / 'multi_signal_summary.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'multi_signal_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("\n" + "="*60)
    print(" PhysNet2DCNN - 生体指標推定（スライディングウィンドウ + 複数指標対応）")
    print("="*60)
    print(f"モード: {config.model_mode}")
    ...
    # 複数指標を順次解析するかどうかの設定
    analyze_multiple_signals = True  # Trueに設定すると複数指標を解析
    ...
    with open(save_dir / 'config_summary.txt', 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("実験設定【スライディングウィンドウ + 複数指標対応版】\n")
        f.write("="*60 + "\n")
    ...
    if config.use_sliding_window:
        print("\n【スライディングウィンドウ】")
        print(f"  10秒ウィンドウで学習・評価を実施")
        print(f"  各タスク60秒を6ウィンドウに分割")
