...
class Config:
    def __init__(self):
        ...
        if self.model_type == "3d":
            ...
            # 軽量化2Dモデル（AMP使用時はバッチサイズ増加可能）
            self.batch_size = 64 if self.use_amp else 32  # AMPでメモリ節約→バッチサイズ増
            self.epochs = 200  # 高速化により多めのエポック
            self.learning_rate = 0.001
            self.weight_decay = 1e-4
            self.patience = 40
...
def create_model(config):
    ...
    if config.verbose:
        ...
        if config.model_type == "3d":
            ...
            print("【推奨】軽量化2Dモデル＋AMPで処理速度が約5-10倍向上します")
...
def main():
    ...
    print("\n" + "="*60)
    print(" PhysNet2DCNN - 個人内解析（6分割交差検証）【高速化版】")
    print("="*60)
